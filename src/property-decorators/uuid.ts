 ...
 * 
 * @param {UUIDOptions} [uuidOptions] Configuration object for UUID decorator
 * @returns {PropertyDecorator}
 */
export function UUID(uuidOptions?: UUIDOptions): PropertyDecorator {
    const protect = (uuidOptions) ? !!uuidOptions.protect : true;
        ...
        Reflect.defineProperty(target, propertyKey, {
            ...
            set: (val: string) => {
                if(val && typeof(val) === 'string' && val.length === 36) {
                    ...
                    if(segments && segments.length === 5 && +(segments[2][0]) === uuidVersion) {
                        value = val;
                    } else {
